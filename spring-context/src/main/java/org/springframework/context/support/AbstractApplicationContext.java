...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// (空的)初始化特定上下文子类中的其他特殊bean。spring boot重写此方法,启动tomcat
				onRefresh();
				...
				// 最后一步:容器刷新,发布相应的事件。(Spring cloud也是从这里启动)
				finishRefresh();
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 初始化 BeanFactory,并进行XML文件读取,并将得到的BeanFactory记录在当前实体的属性中
		refreshBeanFactory();
		// 获取 bean工厂,返回当前实体的 beanFactory 属性
		return getBeanFactory();
	}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 为 beanFactory 增加一个默认的 propertyEditor,这个主要是对bean的属性等设置管理的一个工具类
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));
		
		// 添加 beanPostProcessor, ApplicationContextAwareProcessor 此类用来完成某些 Aware 对象 的注入
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		/**设置要忽略自动装配的接口,很多同学理解不了为什么此处要对这些接口进行忽略,原因非常简单,这些接口的实现是由容器
		 * 通过set方法,所以在使用autowire进行注入的时候需要将这些接口进行忽略*/
		 ...
		 *
		 * aspectJ提供了两种织入方式,第一种是通过特殊编译器,在编译器,将aspectJ语言编写的切面类织入到java类中,第二种是类加载
		 * 期织入，也简称为LTW（Load Time Weaving）
		 */
	...
	/**
	 * 实例化并调用所有注册的BeanFactoryPostProcessor bean，如果给出显式顺序。
	 * <p>必须在单例实例化之前调用。
	 */
	...
	/**
	 * 实例化并注册所有BeanPostProcessor bean，如果给出显式的顺序。
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果之前没有任何bean后处理程序(例如PropertyPlaceholderConfigurer bean)注册，则注册一个默认的嵌入式值解析器:
		// 此时，主要用于解析注释属性值。
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> getEnvironment().resolvePlaceholders(strVal));
		}
		
		// 处理关于aspectj,尽早初始化LoadTimeleaverAware bean，以便尽早注册它们的转换器。
